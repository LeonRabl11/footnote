...
Stand: Schema (`documents`/`chunks`, `vector(1536)` + HNSW/cosine, generierte `content_search` tsvector + GIN), Ingestion (`src/lib/ingestion/*`, Upload `/ingest`, PDF via unpdf + Gemini-OCR-Fallback für Scans), Quellen mit Seite/Zeile. Retrieval (`src/lib/retrieval/*`) ist **hybrid** (Vektor + Volltext, RRF). Antwort ist **agentic**: das Modell ruft die Hybrid-Suche selbst als Tool auf (`search-tool.ts`, `streamText` + `stopWhen: stepCountIs(4)` in `answer.ts`), Quellen werden über alle Suchen gesammelt und via `messageMetadata` an die finish-Nachricht gehängt. Chat-UI auf der Startseite (`useChat` → `/api/chat`). Generierungs-Modell: `GENERATION_MODEL` in `src/lib/embeddings/config.ts`.
